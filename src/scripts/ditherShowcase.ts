...
const boundControls = new WeakSet<HTMLButtonElement>();
...
export const renderDithers = () => {
  document.querySelectorAll<HTMLCanvasElement>('[data-dither-demo]').forEach((canvas) => {
    if (canvas.width === 360 && canvas.height === 120) return;
    process(canvas, canvas.dataset.ditherDemo || 'ordered');
  });

  document.querySelectorAll<HTMLButtonElement>('[data-dither-select]').forEach((button) => {
    if (boundControls.has(button)) return;
    boundControls.add(button);
    const selectMode = () => {
      const mode = button.dataset.ditherSelect as DitherMode;
      if (!modeLabels[mode]) return;

      document.querySelectorAll<HTMLButtonElement>('[data-dither-select]').forEach((control) => {
        const selected = control === button;
        control.setAttribute('aria-pressed', String(selected));
        control.closest<HTMLElement>('[data-dither-preset]')?.classList.toggle('is-active', selected);
      });

      const pipeline = document.querySelector<HTMLElement>('[data-epaper-pipeline]');
      if (!pipeline) return;
      pipeline.dataset.ditherMode = mode;
      pipeline.dispatchEvent(new CustomEvent('peanup:dither-change', {
        detail: { mode, label: modeLabels[mode] },
      }));
    };
